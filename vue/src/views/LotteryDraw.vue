<template>
    <div class="dial" v-wechat-title="$route.meta.title">
        ...
        <!-- 转盘包裹 -->
        <div class="rotate" style="width: 450px;height: 450px">
            <!-- 绘制圆点 -->
            <div
            ...
            <!-- 指针图片 -->
            <img class="pointer" src="../assets/click.png" @click="start" style="width: 6.952rem;height: 10.15rem"/>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                winningIndexDict:{"0":"IphoneX","1":"10元现金","2":"重在参与","3":"30元现金","4":"20元现金","5":"50元现金","6":"5元现金","7":"100元现金"},
                user:{},
                LuckyClick:0,
                circleList: [], //原点设置
        ...
        created() {
            ...
            this.user=localStorage.getItem("user")?JSON.parse(localStorage.getItem("user")):{}
            this.LuckyClick = this.user.count?this.user.count:0
            this.showcircleList();
        },
        ...
        methods: {
            ...
            rotating() {
                ...
                var index = winningIndex.toString()
                var that = this;
                setTimeout(function() {
                ...
                console.log(index)
                this.request.post("/rewardRecordModify",{"name":this.winningIndexDict[index],"userId":this.user.id,"username":this.user.username}).then(res=> {
                    if (res){
                        console.log("成功")
                    }
                })
